...
//	tx := configs.DB.Begin()
//
//	// Defer a rollback if any error occurs
//	defer func() {
//		if r := recover(); r != nil {
//			tx.Rollback()
//		}
...
//	for i, md := range medicineTransaction.MedicineDetails {
//		medicine := schema.Medicine{}
//		if err := tx.First(&medicine, md.MedicineID).Error; err != nil {
//			tx.Rollback()
//			return c.JSON(http.StatusBadRequest, helper.ErrorResponse("Invalid Medicine ID"))
//		}
//
...
//		if err := tx.Model(&medicine).Update("stock", newStock).Error; err != nil {
//			tx.Rollback()
//			log.Println(err)
//			return c.JSON(http.StatusInternalServerError, helper.ErrorResponse("Failed to update Medicine stock"))
//		}
//
//		// Update MedicineTransactionDetails with the calculated total price
//		medicineTransaction.MedicineDetails[i].TotalPriceMedicine = md.Quantity * medicine.Price
//
//		totalPrice += medicineTransaction.MedicineDetails[i].TotalPriceMedicine
//	}
...
//	medicineTransaction.TotalPrice = totalPrice
//
//	// Create the MedicineTransaction record
//	if err := tx.Create(&medicineTransaction).Error; err != nil {
//		tx.Rollback()
//		log.Println(err)
//		return c.JSON(http.StatusInternalServerError, helper.ErrorResponse("Failed to Create Medicine Transaction"))
//	}
